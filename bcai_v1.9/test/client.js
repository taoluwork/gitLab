...
//version: 1.9.2 ,   compatable with sol 1.9.2
//author: taurus tlu4@lsu.edu
//use: $ node client.js --help --version
/////////////////////////////////////////////////////////////////
const version = "bcai_client v1.9.2     ----  by Taurus"
const NetworkID = 512;
...
if(mode == 'user'){
    ...
    if(argv['i'] != undefined) dataID = argv['i'];
}
else if(mode == 'worker'){
...
function userFireMessage(){
    if(argv['stop'] || argv['s'] != undefined) {                               //cancel a request 
        ...
        return myContract.methods.getPendingPool().call().then(function(pool){
            var addr = pool[argv['s']];
            return addr;
        }).then(function(addr){
            return myContract.methods.stopRequest(addr)
        ...
        myContract.methods.updateRequest(time, target, money, dataID)
        .send({from: myAccount, gas: 500000, value: money})
        .then(function(ret){
...
function workerFireMessage(){
    if(argv['C'] != undefined) {                            //complete computation, need validation  
        ...
        return myContract.methods.getProvidingPool().call().then(function(pool){
            var addr = pool[argv['C']];
            return addr;
        }).then(function(addr){
            return myContract.methods.completeRequest(addr, argv['R']).send({from: myAccount, gas:200000})
            .then(function(ret){
        ...
        myContract.methods.updateProvider(maxTime, maxTarget, minPrice)
        .send({from: myAccount, gas: 200000})
        .then(function(ret){
