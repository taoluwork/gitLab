...
contract TaskContract {
    ...
    function updateRequest(uint64 time, uint16 target, uint64 price, uint64 dataID) payable public returns (bool) {      
        if(requestList[msg.sender].status == '0' ){                   //can only update pending request
            requestList[msg.sender].blockNumber    = block.number;         
            requestList[msg.sender].time        = time;       
            requestList[msg.sender].target      = target;
    ...
    function completeRequest(address payable reqAddr, uint64 resultID) public returns (bool) {
