...
contract TaskContract {
    ...
    //ID - counter , +1 each new request or provider
    ...
    uint256[] postProvPool;     //requests  //done the computation, wait to be matched to validate.
...
///////////////////////////////////////////////////////////////////////////////
    ...
    ////// Functions ///////////////////////////////////////////////////////////////////////////////
    // startProviding called by client --worker mode to become a provider. 
    // init new struct on List, Map and Pool. 
    // NOTE: cannot use to update. You must stop a previous one and start a new one. (updateProvider)
    ...
    function startProviding(uint64 maxTime, uint16 maxTarget, uint64 minPrice) public returns (byte) {
        ...
        if(autoAssign) return assignProvider(providerCount-1);  //assign the new provider   
    }
    ...
    function stopProviding(uint256 provID) public returns (bool) {
        ...
        } else return false;                                       //update requirement not met
    ...
    //provider to be update must in the provider pool
    function updateProvider(uint64 maxTime, uint16 maxTarget, uint64 minPrice, uint256 provID) public returns (bool) {      
        bool flag = false;
        flag = ArrayPop(providerPool, provID);              //provider must be in pool
        if(flag && providerList[provID].available == true   //can only modify available provider
    ...
    function startRequest(uint64 time, uint16 target, uint64 price, uint64 dataID) payable public returns (byte) {
        //register on List
        ...
        requestList[requestCount].price         = price;
    ...
    function stopRequest(uint256 reqID) public returns (bool){
        ...
        if (requestList[reqID].status == '0'                       //can only cancel pending request
            ...
            requestList[reqID].status = '7';                       //'done'
        }
        if(flag) 
    ...
    function updateRequest(uint64 time, uint16 target, uint256 reqID) payable public returns (bool) {      
        ...
        && requestList[reqID].addr == msg.sender){          //you cannot modify other's config
            ...
            pendingPool.push(reqID);                        //update pool
            //update map -- no need provID not changed.
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////
    // assignProvider: Search in the pendingPool, find a job for current provider. Triggered by startProviding
    ...
    // assignRequest: Assigning one task to one of the available providers. Only called from startRequest (private)
    ...
    function completeRequest(uint256 reqID, uint64 resultID) public returns (bool) {
        ...
        if (msg.sender == requestList[reqID].provider) {
            ...
            emit SystemInfo(reqID, requestList[reqID].addr, 'Request Computation Completed');
            //start validation process
            if(autoValidate){
                return validateRequest(reqID);
            }else return true;
        }
        else {      //msg.sender != provider
            return false;
        }
    }
    ...
    function validateRequest(uint256 reqID) public returns (bool) {
        ...
        for (uint64 i = 0; i < providerPool.length; i++) {
            ...
            if(validatorsFound < numValidatorsNeeded){
                ...
                emit SystemInfo(reqID, requestList[reqID].addr, 'Enough Validators');
                return true;
                break;
            }
        ...
        emit SystemInfo(reqID, requestList[reqID].addr, 'Not Enough Validators');
    }
    //////////////////////////////////////////////////////////////
    // some cases here:
    ...
    function checkValidation(uint256 reqID) public returns (bool) {
        ...
        emit SystemInfo(reqID, requestList[reqID].addr, 'Validation Complete');
        //popout from pool
